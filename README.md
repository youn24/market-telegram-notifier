...
- 時間外急変は、価格基準・継続性・独立した確認材料2件以上・総合80点以上が重なった場合だけ通知
- テーマ株監視銘柄の日足から、窓開け・高値圏反落・底打ち反転を出来高と翌日確認付きで抽出
...
9:30・17:00通知では、`theme_stocks` に登録した日本株を対象に日足OHLCVを確認します。候補は次の複合条件を満たした場合だけ表示します。

- 上窓または下窓が設定幅以上
- 20日高値・安値の更新と出来高増加、終値位置が一致
- 60日高値・安値圏でハンマー、流れ星、包み足が出現
- 底打ち・高値反落は翌日が前日の高値・安値を超えて確認
- 確認度が `config/sources.yaml` の `price_patterns.minimum_score` 以上

9:30時点では当日途中の未確定日足を除外します。確認度は複合条件の充足度であり、将来の勝率や売買推奨ではありません。欠損データから足型を推測しません。
