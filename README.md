...
- 米国通常取引中も大型株・半導体・日本株ADRを監視し、独立した確認材料が重なる急変だけ通知
...
9:30時点では当日途中の未確定日足を除外します。確認度は複合条件の充足度であり、将来の勝率や売買推奨ではありません。欠損データから足型を推測しません。Telegram本文と詳細ページでは、条件を満たした候補を「株価注意報」と表示します。

## YouTube新着参考情報

17:00通知の詳細ページでは、`config/sources.yaml` の `youtube.channels` に登録したチャンネルの公開情報を確認します。初期設定は「株リアルライブ（あす上がる株）」と「NOBU塾」です。各チャンネルから最大2件ずつ取得するため、片方の投稿だけで表示枠が埋まりません。

- 取得するのは動画タイトル、公開日時、動画URLだけです
- 公開Atomフィードが利用できない場合は公式チャンネルの動画一覧へ切り替え、YouTubeが表示する「○時間前」をそのまま掲載します
- タイトルに明記された語句だけから「日本株・米国株・為替・金利・商品・決算」の参考観点を表示します
- 動画内の予想、銘柄評価、数値は自動で事実認定しません
- 公式開示や取得価格で再確認できるまで、AI分析や株価注意報の根拠には使いません
- 取得できない場合は `未確認` と表示します
- 別のチャンネルを指定する場合は、YouTubeのチャンネルIDを `channel_id` に設定します
- 大引け後レポートには、JPX、日本銀行、財務省の公式確認リンクも掲載し、外部見解より一次情報を優先します
...
- 情報源: 日経、ロイター、Bloomberg、NHK、時事通信、会社四季報オンラインなどを少し優先します
...
17:00通知では、通常検索に加えて大引け後の決算、業績修正、配当、自社株買い、適時開示を追加検索します。発行日時を確認できた直近30時間以内の見出しだけを「大引け後の公開材料」に表示し、見出し語を `好材料語あり` `注意材料語あり` `好悪混在` `方向未分類` に分けます。これは見出しの分類であり、記事本文を推測した売買判断ではありません。

株探は公式ヘルプで掲載情報のシステムによる自動取得を禁止しているため、自動取得・スクレイピングの対象外です。ブラウザ詳細ページには株探ニュースとJPX適時開示情報閲覧サービスへの手動確認リンクだけを表示します。株探の本文を分析したい場合は、利用条件の範囲で利用者が確認した内容を入力してください。
...
- 米国通常取引: NY時間9:30から16:00まで30分ごとに確認（夏時間・冬時間を自動判定、高確認度の急変時だけ通知）
...
`.github/workflows/us_session_alert.yml` が米国通常取引中を30分間隔で確認します。同じ米国取引日の通知は最大1回です。

- 米国大型株: NVIDIA、Broadcom、AMD、Arm、Microsoft、Apple、Amazon、Meta、Alphabet、Tesla
- 市場確認: S&P500、NASDAQ、SOXX、VIX
- 日本株確認: 日経225先物、USD/JPY、トヨタ・ソニー・大手銀行・ホンダのADR
- 銘柄別の入口基準は概ね±3%から±5%、指数は±1.25%から±2%、VIXは±8%です。
- 通知には、総合85点以上、直近4観測のうち3回以上の継続、独立した確認材料2件以上が必要です。
- 確認材料は同業大型株、NASDAQ/S&P500/SOXX、日経先物などです。単独銘柄だけの急騰・急落は通知しません。
- 日経への関連は、半導体、AI・データセンター、電子部品、自動車などの連動確認です。翌日の日経平均への寄与額や方向を断定しません。
- 25分より古いデータ、前営業日終値を確認できないデータ、異常値は採用せず `未確認` とします。

対象銘柄と閾値は `config/sources.yaml` の `us_session`、有効・無効は `config/tasks.yaml` の `us_session_alert.enabled` で変更できます。
