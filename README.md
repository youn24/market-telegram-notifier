...
17:00通知の詳細ページでは、`config/sources.yaml` の `youtube.channels` に登録したチャンネルの公開Atomフィードを確認します。初期設定は「株リアルライブ（あす上がる株）」です。

- 取得するのは動画タイトル、公開日時、動画URLだけです
- 動画内の予想、銘柄評価、数値は自動で事実認定しません
- 公式開示や取得価格で再確認できるまで、AI分析や株価注意報の根拠には使いません
- 取得できない場合は `未確認` と表示します
- 別の「明日上がる株」チャンネルを指定する場合は、YouTubeのチャンネルIDを `channel_id` に設定します
