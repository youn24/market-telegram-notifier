...
def _build_prompt(summary: dict[str, Any]) -> str:
    ...
    prompt = "\n".join(
        [
            ...
            "Write exactly five short labeled lines. Make them decision-useful, not generic commentary.",
            "Line 1 must start with 結論:",
            "Line 2 must start with 根拠:",
            "Line 3 must start with 注視:",
            "Line 4 must start with 反証:",
            ...
            "結論 must state the current stance and immediate time horizon.",
            "根拠 must use the strongest supplied price evidence; prefer two independent markets when available.",
            "注視 must name the next observable confirmation condition, not say only 様子を見る.",
            "反証 must state what observable reversal or disagreement would weaken the conclusion.",
            "Separate confirmed price action from headline context. Never present a headline as the cause unless supplied evidence confirms causality.",
            "",
